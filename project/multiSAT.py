...
if __name__ == "__main__":
    data, metadata = gridsearch_QAOA_SATsolver(
        ...
        num_vars=16,                    # number of qubits/variables in the problem
        p_values=[1, 2, 4, 8],          # QAOA depths
        m_values=np.arange(4, 33, 4),   # number of clauses in the kSAT problem
        n_trials=20,                    # number of random instances to test for each (p, m) pair
        ...
        steps_optim=5000,               # maximum number of optimization steps
        ...
        run_name="test_run6"            # name for this run to save the results under
    )
